...
public class PieceService {
    private final PawnService pawnRepository;
    private final RookService rookRepository;
    private final KnightService knightRepository;
    private final BishopService bishopService;
    private final QueenService queenRepository;
    private final KingService kingRepository;
    ...
    public boolean canTheBishopMove(Board board, CellOnTheBoard start, CellOnTheBoard end, Bishop bishop) {
        return bishopService.canMove(board, start, end, bishop);
    }
    ...
    public synchronized void makeMove(Board board, Move move) {
        ...
        if (pieceOnStart instanceof Pawn && pieceOnEnd != null && pieceOnEnd.isWhite() == pieceOnStart.isWhite()
            ...
            board.getCellOnTheBoardMap()[move.getStart().getLineCoordinate()][move.getStart().getColumnCoordinate()]
                    .setPieces(null);
        } else if (pieceOnStart instanceof King king && !king.isHasBeenMoved() &&
